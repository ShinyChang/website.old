...
(function(window, undefined) {
    ...
    function EpicEditor(options) {
        ...
        var self = this,
            opts = options || {}, _defaultFileSchema, _defaultFile, defaults = {
    ...
    EpicEditor.prototype.load = function(callback) {
        ...
        var self = this,
            ...
            isCtrl = false,
            eventableIframes, i // i is reused for loops
            , boundAutogrow;
        ...
        _HtmlTemplates = {
            ...
            chrome: '<div id="epiceditor-wrapper" class="epiceditor-edit-mode">' + '<iframe frameborder="0" id="epiceditor-editor-frame"></iframe>' + '<iframe frameborder="0" id="epiceditor-previewer-frame"></iframe>' + '<div id="epiceditor-utilbar">' + (self._previewEnabled ? '<button title="' + this.settings.string.togglePreview + '" class="epiceditor-toggle-btn epiceditor-toggle-preview-btn"></button> ' : '') + (self._editEnabled ? '<button title="' + this.settings.string.toggleEdit + '" class="epiceditor-toggle-btn epiceditor-toggle-edit-btn"></button> ' : '') + (self._fullscreenEnabled ? '<button title="' + this.settings.string.toggleFullscreen + '" class="epiceditor-fullscreen-btn"></button>' : '') + '</div>' + '</div>'
        ...
        function shortcutHandler(e) {
            ...
            if (e.keyCode == 9) {
                e.preventDefault();
                self.editorIframeDocument.execCommand('indent', true, null);

                [].forEach.call(self.editorIframeDocument.querySelectorAll("blockquote"), function(blockquote) {
                    var indentLines = [];
                    blockquote.innerHTML.split("<br>").forEach(function(line) {
                        if (e.shiftKey) {
                            indentLines.push(line.replace(/(&nbsp;| ){4}/, ""));
                        } else {
                            indentLines.push("&nbsp; &nbsp; " + line);
                        }
                    });
                    blockquote.outerHTML = indentLines.join("<br>") + "<br>";
                });
            }
